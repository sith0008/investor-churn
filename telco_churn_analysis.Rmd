...
# iterations effective from 5 - 7
telcoChurn.imputed <- rfImpute(Churn ~ ., data = telcoChurn, iter = 6)
```
```{r}
rfmodel <- randomForest(Churn ~ ., data = telcoChurn.imputed, proximity = TRUE)
```
...
for(i in 1:10){
  temp.model <- randomForest(Churn ~ ., data = telcoChurn.imputed, mtry=i, ntree=1000)
...
rfmodeloptim <- randomForest(Churn ~ ., data = telcoChurn.imputed, mtry=2, proximity = TRUE)
```
